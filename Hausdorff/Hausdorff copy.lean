  ...
  · intro X_scat_prop
    ...
    · -- inductive case
      ...
      · have : Scattered lo_lex := by
        ...
        rw [Scattered, <- not_nonempty_iff, <- exists_true_iff_nonempty] at this
        apply this
        use {
          toFun := f'
          inj' := inj
      ...
      · rw [Function.not_injective_iff] at ninj
            ...
            (by
              ...
              · by_contra
                ...
                · rw [h₂] at h₁
                  exact h₁
              · exact h₁.symm)
        let goal := is_scat_f (f x).1
        rw [Scattered, <-not_nonempty_iff] at goal
        apply goal
        let f' : s ↪o f '' s := by exact { -- MAKE IN ISO
        ...
        exact Nonempty.intro (comp_is_orderEmb g' (comp_is_orderEmb f' sQf.toOrderEmbedding))
        ...
        have h₁ {q : s} : (ind (f q).fst) = (ind (f x).fst) := by sorry
        let i : s ↪o ind (f x).1 := {
          toFun := fun q => h₁ ▸ (f q).2
          inj' := by
            intro x' y' h'
            simp at h'
            apply SetCoe.ext (@f.inj' x' y' ?_)
            sorry
          map_rel_iff' := sorry
        }

        let s := {z : (ind (f x).1) | ⟨(f x).1, z⟩  ∈ Set.range f}
        have : Scattered (LinOrd.mk s) := by
          by_contra h
          rw [Scattered, <-not_nonempty_iff, not_not] at h
          rcases h with ⟨g⟩
          let p := is_scat_f (f x).1
          rw [Scattered, <-not_nonempty_iff, <-exists_true_iff_nonempty] at p
          apply p
          use {
            toFun := fun x => g x
            inj' := by
              intro x y h
              apply g.inj' (SetCoe.ext h)
            map_rel_iff' := by
              intro x y
              simp }
        -- apply Scattered_to_subset_props (ind (f x).1) (is_scat_f (f x).1)
        --   {z | ⟨(f x).1, z⟩ ∈ Set.range f ∧ (f x).2 < z ∧ z < h.left ▸ (f y).2}
        apply Scattered_to_subset_props (LinOrd.mk s) this
          {z | (f x).2 < z ∧ z < ⟨(h.left ▸ (f y).2),
                                    by
                                    simp [s]; use y
                                    sorry ⟩}
        split_ands
        · apply @Subtype.countable s ?_
          have : Countable (Set.range f) := by sorry
          apply?
        sorry
