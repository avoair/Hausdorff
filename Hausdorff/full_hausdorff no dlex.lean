...
| lex_sum (WO: LinOrd)
  ...
  (h : L ≃o ({carrier := Σₗ w, (f w).carrier,
              str := Sigma.Lex.linearOrder} : LinOrd)): IScattered_prop L
...
lemma iscat_of_well_founded (X: LinOrd.{u}) (h : WellFounded X.str.lt) : IScattered_prop.{u,u} X := by
  ...
  have : X ≃o L := by
    ...
    · intro h1
      ...
      · left; simp; exact h1
      · right; use h1
  let p := (IScattered_prop.lex_sum X (Or.inl h)
  ...
  apply IScattered_prop.lex_sum X (Or.inl h)
      (fun x => {carrier := PUnit, str := inferInstance})
      (fun x => by
        simp
        apply IScattered_prop.base PUnit.unit
...
--   IScattered_prop.{u} {carrier := ({x} : Set α), str := inferInstance} := by
...
lemma iscat_of_layered_iscats (L M N : LinOrd.{u}) (h1 : IScattered_prop.{u, u} M)
  ...
  IScattered_prop.{u, u} L := by
  apply IScattered_prop.lex_sum Two.L (Or.inl (@Finite.to_wellFoundedLT _ Two.finite).1) (g' M N)
  · intro w
    match w with
...
lemma iscat_of_subset {L : LinOrd.{u}} (A : Set L) (h : IScattered_prop.{u, u} L) :
  IScattered_prop.{u, u} { carrier  := A, str := inferInstance} := by
...
lemma iscat_of_subset' {L : LinOrd.{u}} (A : Set L)
  (h : IScattered_prop.{u, u} { carrier := A, str := inferInstance}) (B : Set L) (h1 : B ⊆ A) :
...
lemma iscat_of_subset'' {L : LinOrd.{u}} (P : L → Prop) (T : L → Prop)
  (h : IScattered_prop.{u, u} { carrier := {x | P x}, str := inferInstance}) :
  IScattered_prop.{u, u} { carrier := {x : {x // T x} | P x}, str := inferInstance} := by
...
lemma iscat_of_rev_iscat {L : LinOrd.{u}} (h : IScattered_prop.{u,u} L) :
   IScattered_prop.{u,u} {carrier := L.carrier , str := L.str.swap} := by
  induction' h with M m hm WO hWO f f_scat L Liso IH
  · apply IScattered_prop.base
    exact hm
  · let map (w : ({ carrier := WO.carrier, str := WO.str.swap } : LinOrd)) : LinOrd :=
    ...
    apply IScattered_prop.lex_sum { carrier := WO.carrier, str := WO.str.swap } ?_ map IH _ iso
...
lemma ind_scat_of_fin_seg_of_one_class {X : LinOrd.{u}}
  (x : X) (one_class : ∀ x y : X, iscat_int x y) : IScattered_prop.{u,u} {carrier := {x_1 // x < x_1}, str := inferInstance} := by
...
theorem Hausdorff_Scattered_Orders (X : LinOrd.{u}): IsScattered X ↔ IScattered_prop.{u,u} X := by
  ...
  · intro X_scat
    ...
    · rcases Classical.exists_true_of_nonempty nonempt with ⟨x⟩
      have part1 : IScattered_prop.{u,u} {carrier := {x_1 // x < x_1}, str := inferInstance} := by
        exact @ind_scat_of_fin_seg_of_one_class _ x one_class
      have part2 : IScattered_prop.{u,u} {carrier := {x_1 // x_1 < x}, str := inferInstance} := by
