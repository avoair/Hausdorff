...
lemma iscat_of_WF_parition (L : LinOrd.{u}) (Cof : Set L)
  ...
  (j : Cof → Set L) (j_partition : ∀ z : L, ∃! (a : Cof), z ∈ j a)
  (j_ordered : ∀ (a b : Cof), a < b → ∀ a_1 ∈ j a, ∀ b_1 ∈ j b, a_1 < b_1)
  ...
  let Jiso : L ≃o ({carrier := Σₗ (w : Cof), (J w), str := Sigma.Lex.linearOrder} : LinOrd) :=
    @iso_of_sigma_partition L S1 j j_partition j_ordered
  apply Scattered_ind_prop.lex_sum.{u,u} S1
    ?_ J ?_ _ Jiso
  · left; exact wellFounded_lt
  · exact fun w ↦ all_iscat w

lemma dense_quotient {L : LinOrd} {a b : L} (h : ¬ iscat_int a b) (hl : a < b) :
  ...
  · rcases @exists_cof_WF_subset A inferInstance with ⟨Cof, hCof⟩
    ...
    apply iscat_of_WF_parition _ Cof hCof
...
lemma ind_scat_of_fin_seg_of_one_class {X : LinOrd.{u}}
  ...
  · let j (a : Cof) := {z : {x_1 // x < x_1} | z ≤ a ∧ ∀ a' < a, a' < z}
    apply iscat_of_WF_parition (LinOrd.mk {x_1 // x < x_1}) Cof hCof j
    -- PARTITION
    · intro z
      let above := {a : Cof | z ≤ a}
