...
lemma iscat_int_in_L_swap {L : LinOrd} (a b : L) : @iscat_int L a b ↔ @iscat_int (LinOrd_swap L) a b := by sorry
...
lemma dense_quotient {L : LinOrd.{u}} {a b : L} (h : ¬ iscat_int a b) (hl : a < b) :
  ...
  · simp [B]
    ...
    have : Scattered_ind_prop.{u,u} B' := by
      -- let P : LinOrd := LinOrd.mk { x : (LinOrd_swap) L | @LT.lt (LinOrd_swap L) _ b x
      --                                         ∧ @LE.le (LinOrd_swap L) _ x a ∧ iscat_int b x}
      -- let iso : @OrderIso B' P B'.str.toLE P.str.toLE := by
      --   exact {
      --   toFun := fun x => ⟨x,
      --     by
      --     change b > x ∧ x ≥ a ∧ iscat_int b x
      --     exact And.intro x.2.right.left
      --           (And.intro x.2.left (scat_int_symm x.2.right.right))⟩
      --   invFun := fun x => ⟨x,
      --     by
      --     change x ≥ a ∧ b > x ∧ iscat_int x.1 b
      --     exact And.intro x.2.right.left
      --           (And.intro x.2.left (scat_int_symm x.2.right.right))⟩
      ...
      exact {
        ...
        invFun := fun x => ⟨x,
          ...
          · exact x.2.left
          · apply scat_int_symm
            apply (iscat_int_in_L_swap x.1 b).mp (x.2.right.right)⟩
        left_inv := by intro _; trivial
        right_inv := by intro _; trivial
        map_rel_iff' := by
          intro x y
          simp [B']
          have : x.1 ≤ y.1 ↔ @LE.le { x // b < x ∧ x ≤ a ∧ iscat_int b x } Subtype.le x y  := by
            simp
          rw [<-this]
