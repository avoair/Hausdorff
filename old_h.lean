...
-- the easy direction that doesnt ignore isomorphisms
...
lemma Scat_iff_not_embeds_Q (X : LinOrd) : IsScattered X ↔ IsEmpty (ℚ ↪o X) := by
  ...
  · intro h
    ...
    have : ∃ A : Set X.α, Nonempty (ℚ ≃o A) := by
      use ↑(Set.range ⇑f)
...
| base {L : LinOrd.{u}} (x : L.α) (h : {x} = @Set.univ L.α) : Scattered_ind_prop L
| lex_sum (WO: LinOrd.{u})
          (hwo : WellFounded WO.str.lt
                ∨ WellFounded (LinearOrder.swap WO.α WO.str).lt) --- could split into two cases
          (f: WO.α → LinOrd)
          (h : ∀ w, Scattered_ind_prop (f w))
          (L : LinOrd.{u})
          (h : L ≃o ({α := Σₗ w, (f w).α, str := Sigma.Lex.linearOrder} : LinOrd)): Scattered_ind_prop L
...
lemma Rev_Scattered_of_Scattered (X : LinOrd) : IsScattered X → IsScattered ⟨X.α, LinearOrder.swap X.α X.str⟩ := by
  intro X_Scat
...
lemma  help (X: LinOrd) : -- X.swap.swap = X := by
  LinearOrder.swap X.α (LinearOrder.swap X.α X.str) = X.str := by
  ext
  rfl

lemma Rev_Well_Ord_Scattered (X : LinOrd) : WellFounded (LinearOrder.swap X.α X.str).lt → IsScattered X := by
  intro h
  have p := Rev_Scattered_of_Scattered ⟨X.α, LinearOrder.swap X.α X.str⟩ (Well_Ord_Scattered ⟨X.α, LinearOrder.swap X.α X.str⟩ h)
  simp [help X] at p
  exact p

lemma Rev_Well_Ord_Scattered (X : LinOrd) : WellFounded (LinearOrder.swap X.α X.str).lt → IsScattered X := by -- rw with above-- one liner ish
  intro hwf
  intro A LO props
  rcases props.right.right.right.right with ⟨y, hy⟩
  rcases Decidable.em (Nonempty ↑A) with nonempt | empt
...
-- ↓↓ difficulty of the other direction
#check PUnit
lemma Well_Ord_ind_scat_prop (X: LinOrd.{u}) (h : WellFounded X.str.lt): Scattered_ind_prop X := by
  let L : LinOrd := { α := Σₗ (w : ↑X), (fun w => (↑{ α := PUnit.{u+1}, str := inferInstance } : LinOrd)) w,
  ...
  have : X ≃o L := by
    ...
    intro a b
    simp [Sigma.Lex.le_def, <-le_iff_lt_or_eq]
  apply Scattered_ind_prop.lex_sum X (Or.inl h)
    (fun x => {α := PUnit, str := inferInstance})
    (fun x => by
      simp
...
-- def scat_int {L : LinOrd} (x y : L) : Prop :=
--   let M : LinOrd := {α := {a | (x ≤ a ∧ a ≤ y) ∨ (y ≤ a ∧ a ≤ x)}, str := inferInstance}
--   Scattered_ind_prop M
def scat_int {L : LinOrd} (x y : L) : Prop :=
  if ∃ h : x < y, true then
    let M : LinOrd := {α := {a | x ≤ a ∧ a ≤ y}, str := inferInstance}
    Scattered_ind_prop M
  else
    let M : LinOrd := {α := {a | y ≤ a ∧ a ≤ x}, str := inferInstance}
    Scattered_ind_prop M
...
lemma scat_int_equiv {L : LinOrd}: Equivalence (@scat_int L) := by
  ...
  · intro x
    simp [scat_int]
    apply (Well_Ord_ind_scat_prop { α := { x_1 // x ≤ x_1 ∧ x_1 ≤ x}, str := inferInstance })
    ...
    #check (@Finite.to_wellFoundedLT _ this _).1
    sorry -- its so close -- inferences just dont quite line up

  · intro x y h
    simp [scat_int] at h
    simp [scat_int]
    rcases lt_or_ge x y with h1 | h1
    · rw [ite_prop_iff_or] ; rw [ite_prop_iff_or] at h
      right; constructor
      · simp; exact le_of_lt h1
      · rcases h with h2 | h2
        · exact h2.right
        · by_contra _
          exact h2.left h1
    · rw [ite_prop_iff_or]; rw [ite_prop_iff_or] at h
      rcases h with h' | h'
      · by_contra _; exact (not_lt_of_ge h1) h'.left
      · rcases lt_or_eq_of_le h1 with h2 | h2
        · left; exact And.intro h2 h'.right
        · right; constructor
          · exact not_lt_of_ge (ge_of_eq h2)
          · subst x ; exact h'.right
  · intro x y z hxy hyz
    simp [scat_int];  simp [scat_int]
