    ...
    · rcases Classical.exists_true_of_nonempty nonempt with ⟨x⟩
      ...
      have part1 : Scattered_ind_prop.{u,u} {carrier := {x_1 // x < x_1}, str := inferInstance} := by
        ...
        · let J (a : S1) :=
          ...
          have J_cover (z : {x_1 // x < x_1}) : ∃ (a : S1), z ≤ a ∧ ∀ a' < a, a' < z := by
            let above := {a : S1 | z ≤ a}
          ...
          have Jiso : ({ carrier := { x_1 // x < x_1 }, str := inferInstance } : LinOrd) ≃o
                      ({ carrier := Σₗ (w : ↑S1), ↑(J w), str := Sigma.Lex.linearOrder } : LinOrd) :=
            {
              toFun := fun x => ⟨choose (J_cover x), ⟨x,choose_spec (J_cover x)⟩⟩
              invFun := fun x => x.2.1
              left_inv := by intro x; simp
              right_inv := by
                intro x
                simp
                have : choose (J_cover x.2) = x.1 := by
                  #check choose_spec (J_cover x.2)
                  #check x.2.2
                  sorry

                -- to show it is an isomorphism, must show that choose is deterministic
                -- i.e. above is unique, but
