...
def decomp (WO : LinOrd) (f : WO → LinOrd) (a : ({carrier := Σₗ (w : ↑WO), ↑(f w), str := Sigma.Lex.linearOrder} : LinOrd)):
  LinOrd := {carrier := Σₗ (w : Two), {x :({carrier := Σₗ (w : ↑WO), ↑(f w), str := Sigma.Lex.linearOrder} : LinOrd)
              | (w = 0 ∧ x.1 < a.1) ∨ (w = 1 ∧ x.1 = a.1 ∧ x ≤ a)}, str := Sigma.Lex.linearOrder }

def decomp_iso (WO : LinOrd) (f : WO → LinOrd) (a : ({carrier := Σₗ (w : ↑WO), ↑(f w), str := Sigma.Lex.linearOrder} : LinOrd)):
  decomp WO f a ≃o ({ carrier := ↑{x | x ≤ a}, str := inferInstance } : LinOrd) := by
  use {
    ...
    --     · --simp
    --       sorry
    --     · rcases x.2.2 with h1 | h1
    --       · exact Eq.symm h1.left
    --       · by_contra
    --         exact not_lt_of_le (le_of_eq (Eq.symm h1.right.left)) h
    --   · apply Sigma.eq
    --     · --simp
    --       sorry
    --     · simp
  ...
  constructor
  · intro h
    simp [decomp, Sigma.Lex.le_def]
    rcases x.2.2 with h1 | h1
    · rcases Two_def y.1 with h2 | h2
      · right; use Eq.trans h1.left (Eq.symm h2)
        constructor -- somehow this changes the goal... from le to lt
        --exact h -- dont know why this doesnt work
        sorry
      · left; rw [h1.left, h2]; exact Fin.zero_lt_one
    · rcases Two_def y.1 with h2 | h2
      · by_contra
        simp [Sigma.Lex.le_def] at h
        rcases x.2.2 with hx | hx
        · exact zero_ne_one ( Eq.trans (Eq.symm hx.left) h1.left)
        · rcases y.2.2 with hy | hy
          · rcases h with h3 | h3
            · rw [hx.right.left] at h3
              exact lt_irrefl a.1 (lt_trans h3 hy.right)
            · rcases h3 with ⟨h3⟩
              rw [<-h3, hx.right.left] at hy
              exact lt_irrefl a.1 hy.right
          · exact zero_ne_one (Eq.symm ( Eq.trans (Eq.symm hy.left) h2))
      · right; use Eq.trans h1.left (Eq.symm h2)
        constructor
        · sorry
        --exact h -- dont know why this doesnt work
  · intro h
    simp [decomp, Sigma.Lex.le_def] at h
    rcases h with h1 | h1
    · have : x.1 = 0 ∧ y.1 = 1 := by
        exact Two_le x.1 y.1 h1
      rcases x.2.2 with hx | hx
      · rcases y.2.2 with hy | hy
        · by_contra; exact zero_ne_one (Eq.trans (Eq.symm hy.left) this.right)
        · constructor
          rw [hy.right.left]
          exact hx.right
      · by_contra; exact zero_ne_one (Eq.trans (Eq.symm this.left) hx.left)
    · sorry--constructor -- again constructor converts le to lt... and particularly of the first coordinate
      -- sorry
      -- sorry
...
lemma scat_int_seg {L : LinOrd.{u}} (a : L) (h : Scattered_ind_prop.{u, u} L) : Scattered_ind_prop.{u, u} {carrier := {x // x ≤ a}, str := inferInstance} := by
  ...
  · apply @scat_ind_of_iso { carrier := {x // x ≤ a}, str := inferInstance }
    ...
    -- let decomp_f : Two.{u} → LinOrd :=
    --   g' ({carrier := {x : ({carrier := Σₗ (w : ↑WO), ↑(f w), str := Sigma.Lex.linearOrder} : LinOrd.{u}) |
    --   x.1 < (Niso a).1} , str := inferInstance} : LinOrd.{u})
    --     ({carrier := {x : ({carrier := Σₗ (w : ↑WO), ↑(f w), str := Sigma.Lex.linearOrder} : LinOrd.{u}) |
    --       x.1 = (Niso a).1 ∧ x ≤ (Niso a)} , str := inferInstance} : LinOrd.{u})

    -- let decomp : LinOrd.{u} :=
    --   {carrier := Σₗ (w : Two),
    --     {x : ({carrier := Σₗ (w : ↑WO), ↑(f w), str := Sigma.Lex.linearOrder} : LinOrd)
    --           | (w = 0 ∧ x.1 < (Niso a).1) ∨ (w = 1 ∧ x.1 = (Niso a).1 ∧ x ≤ (Niso a))},
    --     str := inferInstance }

    -- have decomp_iso : decomp ≃o ({ carrier := ↑{x | x ≤ Niso a}, str := inferInstance } : LinOrd) :=
    -- {
    --   toFun := fun x => ⟨x.2.1, by
    --                             rcases x.2.2 with h | h
    --                             · simp at h
    ...
    --                               exact Or.inl h.right
    --                             · apply h.right.right ⟩
    --   invFun := fun x => if h : x.1.1 < (Niso a).1
    --     then ⟨0, ⟨x.1, by left; simp; exact h⟩⟩
    --     else ⟨1, ⟨x.1, by
    ...
    · let helper1 (x : ({ carrier := ↑{x | x ≤ Niso a}, str := inferInstance } : LinOrd))
      ...
      -- let k : ({ carrier := ↑{x | x ≤ Niso a}, str := inferInstance } :LinOrd)
      --         ≃o ({ carrier := Σₗ (w : ↑Two), ↑(g (Niso a) w), str := Sigma.Lex.linearOrder } : LinOrd) :=
      --   {
      --     toFun := fun x => if ∃ (h : x.1.1 = (Niso a).1), true
      --                       then ⟨1, helper2 x _⟩
      --                       else ⟨0, helper1 x _⟩
      --     invFun := sorry
      --     left_inv := by sorry
